...
namespace rendering {
    mesh::mesh(core::engine& engine, renderer& renderer, const std::vector<char>& source, const std::string& file_name) noexcept
    : vbo_{GL_STATIC_DRAW}, ibo_{GL_STATIC_DRAW}, parallax_mapping_{false} {
        ...
        if (head.magic != 0x31424D5A) {
            throw std::runtime_error(file_name + " is not an rendering mesh file");
        }
