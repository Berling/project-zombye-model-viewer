...
namespace rendering {
    skinned_mesh::skinned_mesh(core::engine& engine, renderer& renderer, const std::vector<char>& source, const std::string& file_name) noexcept
    : vbo_{GL_STATIC_DRAW}, ibo_{GL_STATIC_DRAW}, parallax_mapping_{false} {
        ...
        for (auto i = 0; i < head.submesh_count; ++i) {
            ...
            s.diffuse = engine.graphics_system().texture_manager().load("texture/" + texture_name);
            if (!s.diffuse) {
            ...
            s.normal = engine.graphics_system().texture_manager().load("texture/" + texture_name);
            if (!s.normal) {
                throw std::runtime_error("could not load texure " + texture_name);
            }
            data_ptr += sizeof(uint64_t);
            ...
            texture_id = *reinterpret_cast<const uint64_t*>(data_ptr);
            texture_name = std::to_string(texture_id) + ".dds";
            s.material = engine.graphics_system().texture_manager().load("texture/" + texture_name);
            if (!s.material) {
                throw std::runtime_error("could not load texure " + texture_name);
            }
            data_ptr += sizeof(uint64_t);
        ...
        renderer.skinnedmesh_layout().setup_layout(vao_, &vbo_);
    }
